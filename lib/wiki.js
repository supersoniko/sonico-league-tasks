...
const LEAGUE_REGION_RE = /\{\{LeagueRegion\|([^}]+)\}\}/gi;
const LEAGUE_ISSUE_RE = /\{\{Equilibrium League Issue\|([^}]+)\}\}/gi;
const WIKILINK_RE = /\[\[(?:[^|\]]+\|)?([^\]]+)\]\]/g;
...
function parseLeagueRegions(text) {
  return [...String(text || "").matchAll(LEAGUE_REGION_RE)].map((m) =>
    m[1].trim().toLowerCase()
  );
}

function parseLeagueIssues(text) {
  return [...String(text || "").matchAll(LEAGUE_ISSUE_RE)].map((m) =>
    m[1].trim()
  );
}
...
export function parseTaskCatalog(raw) {
  ...
  for (const row of rows) {
    ...
    const leagueIssues = parseLeagueIssues(
      `${meta.other || ""} ${info} ${name}`
    );
    const requiredRegions = parseLeagueRegions(meta.other || "");
