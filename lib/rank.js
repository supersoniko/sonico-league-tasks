...
import { inferRequiredQuests, scoreDeltaForQuests, hasQuestGate } from "./quests.js";
 ...
 */
const HINT_BOOST = {
  // No boost for quest-gated Bill beer — New Foundations is long
  283: 50, // whirligig — no items/quests
  295: 50, // rose
  598: 45, // karambwanji
  603: 45, // stepping stones
  596: 45, // brimhaven ticket
  292: 45, // scarabs
  // earth altar / water rune need talisman drops — not Elriss shop
  99: 0,
  39: 0,
  811: 10,
  812: 10,
...
function baseEasinessScore(task, quests = []) {
  const tierBase = {
    easy: 1000,
    medium: 700,
    hard: 400,
  ...
  const questGated = hasQuestGate(quests);

  // Quest-gated talk/give/equip is NOT free — skip instant boost
  if (!questGated) {
    if (INSTANT_RE.test(text) && !GRIND_RE.test(text)) score += 80;
    else if (INSTANT_RE.test(text)) score += 35;
    if (/\b(one tile|a rose|a ticket)\b/i.test(text)) score += 25;
  } else {
    score -= 40; // explicit penalty vs looking "instant"
  }
...
export async function filterAndRankTasks(tasks, player) {
  ...
  for (const { task, requiredItems, requiredQuests } of candidates) {
    ...
    const score =
      baseEasinessScore(task, requiredQuests) + itemDelta + questDelta;
